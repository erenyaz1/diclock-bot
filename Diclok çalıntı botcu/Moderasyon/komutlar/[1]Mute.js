...
exports.run =  async (client, message, args) => {
  ...
  member.roles.add(mutelirolu)
  db.add(`yetkili.${message.author.id}.mute`, 1)
db.add(`yetkili.${message.author.id}.toplam`, 1)
  let cezaID = db.get(`cezaid.${message.guild.id}`)+1
  db.add(`cezaid.${message.guild.id}`, +1);
  db.push("tempmute", { id: member.id, bitis: Date.now()+ms(time) });
  db.set(`punishments.${cezaID}.${message.guild.id}`, { mod: message.author.id, sebep: sebep, kisi: member.id, id: cezaID, zaman: Date.now(), komut: "METİN-SUSTURMA" });
  db.set(`mstatus.${member.id}.${message.guild.id}`, true);
  db.push(`sicil.${member.id}.${message.guild.id}`, { mod: message.author.id, sebep: sebep, id: cezaID, zaman: Date.now(), komut: "METİN-SUSTURMA" });
  ...
  .setDescription(`**${message.author}, ${member} kullanıcısını \`\`${sebep}\`\` nedeni ile \`\`${args[1].replace(/s/, ' Saniye').replace(/m/, ' Dakika').replace(/h/, ' Saat').replace(/d/, ' Gün')}\`\` boyunca metin kanallarından susturdu. [Ceza Numarası: \`#${cezaID}]\`**`)
  .setColor('#5e0a0a')
  .setFooter(footer)
message.channel.send(mutelendin).then(x => x.delete({ timeout: 12000}))  
  ...
  .setTitle('Bir Kişi Metin Kanallarından Mutelendi')
  .setColor('#5e0a0a')
  .setTimestamp()
  .setDescription(`
  ...
  **Sebep:** [\`\`${sebep}\`\`]`)
  ...
  setTimeout(function(){
    member.roles.remove(mutelirolu)
  }, ms(yaziSure));
}
exports.conf = {
    enabled: true,
